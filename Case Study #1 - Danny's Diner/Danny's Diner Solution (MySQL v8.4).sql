...
FROM
  (SELECT product_name,
          COUNT(s.product_id) AS order_count,
          RANK() OVER( ORDER BY COUNT(s.product_id) DESC) AS order_rank
   FROM menu AS m
	INNER JOIN sales AS s 
	ON m.product_id = s.product_id
...
-- Asssumption: Since timestamp of purchase is not available, purchase made when order_date is the same or after 
-- the join date is used to find the first item
WITH diner_info AS
  (SELECT 
	  product_name,
      s.customer_id,
